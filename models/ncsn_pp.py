...
def init_layer_with_kaiming_uniform(layer):
    """Initializes the linear or conv2d layer with kaiming normal weights"""
    if not isinstance(layer, (nn.Linear, nn.Conv2d)):
        raise RuntimeError("Attempted to initialize a layer with kaiming, that was not nn.Linear or nn.Conv2d")
    nn.init(layer.weight, mode="fan_in")
    if layer.bias is not None:
        nn.init(layer.bias, mode="fan_in")


class ScaledDotProdAttention(nn.Module):
    def __init__(self,):
        """There are no learnable params, this is just an operation.
        Also note that this only performs the operation up to softamx, but doesn't do
        softmax(QK^T)/sqrt(d)*V"""
        super().__init__()
        
    def forward(self, query, key):
        """Attention operation"""
        dim = query.shape[1] # first dim is batch
        """we can use torch.einsum for batch matrix mul
        see: https://pytorch.org/docs/stable/generated/torch.einsum.html"""
        return nn.functional.softmax(query@key.transpose())
#----------------------------------------------------------------------------
